...
BOOL __attribute__((optimize("O0"))) imports_init()
{
    FARPROC(WINAPI * get_proc_address)(HMODULE, LPCSTR) =
        (void*)imports_get_proc_address(LoadLibraryExW_p(L"kernel32.dll", NULL, 0), "GetProcAddress");

    if (!get_proc_address)
        get_proc_address = (void*)imports_get_proc_address(LoadLibraryExW_p(L"kernelbase.dll", NULL, 0), "GetProcAddress");

    if (!get_proc_address)
        return FALSE;

    HMODULE(WINAPI * load_library)(LPCSTR) =
        (void*)get_proc_address(LoadLibraryExW_p(L"kernel32.dll", NULL, 0), "LoadLibraryA");
